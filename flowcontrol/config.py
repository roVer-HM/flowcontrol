...
def read_config_file():
    ...
    if not os.path.isdir(source_path):
        raise ValueError(
            f"File {config_file_path} does not exist. Use 'echo $CROWNET_HOME > ~/.flowcontrolconfig' to generate the file automatically, see https://sam-dev.cs.hm.edu/rover/flowcontrol"
        )
...
if __name__ == "__main__":
    ...
    add_env_variables()
    print(
        f"Add {crownet_env}={os.environ[crownet_env]} and {vadere_env}={os.environ[vadere_env]} to enviroment variables."
    )
